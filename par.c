...
#define DIM 1500					// lateral dimension of the map
...
//		#pragma omp parallel for ordered private(i,c) shared(map,fp) reduction(+: free_cell_num)
		for(i=0; i<DIM*DIM; i++){
			if(!feof(fp)){
...
void initCells(Cell arrayCells[], int start[], int goal[]){	
	...
	double startT,endT;
	startT = omp_get_wtime();
	#pragma omp parallel for schedule(dynamic) private(i,j,pos) shared(arrayCells)		// mi pare che faccia risparmiare meno di un secondo intorno alle 10.000, ma poi aumentando aggiunge un grosso overhead...
	for (i = 0; i < DIM; i++) {
		for (j = 0; j < DIM; j++) {
			pos = i*DIM + j;
...
*/		
		#pragma omp parallel for shared(arrayCells, openSet) reduction(||:isThereBest) reduction(min:tmpF)
		for (int i = 0; i < openSetSize; i++) {
		//	printf("OK ");
		...
		if((foundPath && !isThereBest) || openSetSize == 0){
			...
			printf("Tempo nel ciclo di ricerca Best: %f\nTempo nel ciclo di rimozione da openSet: %f", t1, t2);
...
//	omp_set_num_threads(24);			//	ATTENZIONE A QUESTO VALORE
