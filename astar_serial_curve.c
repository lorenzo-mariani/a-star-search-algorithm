...
// check a change of direction in the path
bool isChanged(Cell arrayCells[], int path[], int pathSize, int bpPos){
	bool isChanged = true;
	int tm0 = path[pathSize-1];
	int tm1 = path[pathSize-2];
	int tm2 = path[pathSize-3];
	
	if( arrayCells[tm0].row == arrayCells[tm1].row && arrayCells[tm0].row == arrayCells[tm2].row)
		isChanged = false;
	else if( arrayCells[tm0].col == arrayCells[tm1].col && arrayCells[tm0].col == arrayCells[tm2].col )
		isChanged = false;
	else if( arrayCells[tm0].row == arrayCells[tm1].row-1 && arrayCells[tm0].row == arrayCells[tm2].row-2 && arrayCells[tm0].col == arrayCells[tm1].col-1 && arrayCells[tm0].col == arrayCells[tm2].col-2 )
		isChanged = false;
	else if( arrayCells[tm0].row == arrayCells[tm1].row-1 && arrayCells[tm0].row == arrayCells[tm2].row-2 && arrayCells[tm0].col == arrayCells[tm1].col+1 && arrayCells[tm0].col == arrayCells[tm2].col+2 )
		isChanged = false;
	else if( arrayCells[tm0].row == arrayCells[tm1].row+1 && arrayCells[tm0].row == arrayCells[tm2].row+2 && arrayCells[tm0].col == arrayCells[tm1].col-1 && arrayCells[tm0].col == arrayCells[tm2].col-2 )
	...
	while (1) {
		...
		if (is_goal(c, goal)) {					// check whether the current cell is the goal point or not
			...
			while ((arrayCells[thisCellPos].parentRow != -1) && (arrayCells[thisCellPos].row != arrayCells[thisCellPos].parentRow || arrayCells[thisCellPos].col != arrayCells[thisCellPos].parentCol)) {
				...
				if(pathSize > 2){
					if ( isChanged(arrayCells, path, pathSize, bpPos) ){
						printf("una curva! ");
						changes++;
					}
				}
			...
			if(!foundPath || (foundPath && arrayCells[path[0]].f < arrayCells[bestPath[0]].f) || (foundPath && arrayCells[path[0]].f == arrayCells[bestPath[0]].f && changes < bestChanges)) {
