...
#define DIM 1000					// lateral dimension of the map
...
void initCells(Cell arrayCells[], int start[], int goal[]){	
	...
	double startT,endT;
	startT = omp_get_wtime();
	#pragma omp parallel for schedule(dynamic) private(i,j,pos) shared(arrayCells)
	for (i = 0; i < DIM; i++) {
		for (j = 0; j < DIM; j++) {
			pos = i*DIM + j;
...
void search (bool map[], int start[], int goal[]) {	
	...
	while (1) {
		...
		if((foundPath && !isThereBest) || openSetSize == 0){
			...
			printf("Tempo nel ciclo di ricerca Best: %f\nTempo nel ciclo di rimozione da openSet: %f\n", t1, t2);
	...
	//		printf("Cell: %d,%d     Neighbor: %d,%d\n", c[0], c[1], neighbor[0], neighbor[1]);
